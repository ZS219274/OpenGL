...
void initModel()            // 初始化模型数据函数
{
  ...
  // 设置顶点属性指针，指定如何解析顶点数据
  glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void *)0);        // 位置属性(前3个float)
  glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void *)(sizeof(float) * 3)); // 颜色属性(后3个float)
...
void initTexture()
{
  picImage = ffImage::readFromfile("resource/pic.png");
}
