...
glm::mat4 _viewMatrix(1.0f);  // view单位矩阵
glm::mat4 _projMatrix(1.0f);  // 投影单位矩阵
...
void rend() // 渲染函数
{
  ...
  for (int i = 0; i < 10; i++)
  {
    glm::mat4 _modelMatrix(1.0f); // model单位矩阵
    _modelMatrix = glm::translate(_modelMatrix, modelVecs[i]); //右边叠加平移
    _modelMatrix = glm::rotate(_modelMatrix, glm::radians((float)glfwGetTime() * (i+1) * 10), glm::vec3(0.0f, 1.0f, 0.0f)); // 右边叠加旋转
